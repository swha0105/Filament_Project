...
for box_length in box_length_list:
    for box_num in box_num_list:
        ...
        grid_mpc = round(int(box_length[:3])/resolution,1)

        minimum_length = int(distance/4)
        smooth_scale = int(distance/5) 
            ...
            if not os.path.isdir(filament_path):
                ...
                for filament_num in np.array(sorted(os.listdir(filament_path),key=int)):
                    ...
                    if length_filament < minimum_length:
                        ...
                        filament_smooth = np.array(list(zip(tmp_filament[:,0],tmp_filament[:,1],tmp_filament[:,2])))
                ...
                for num in range(len(coords_fit)):
                    ...
                    for i in range(len(coords)-smooth_scale):
                        P = coords[i:i+smooth_scale,:]

                    
                        if (P[0,2] == P[:,2]).all() and (P[0,1] == P[:,1]).all():
                            curvature = 0
                            pass
                        elif (P[0,1] == P[:,1]).all() and (P[0,0] == P[:,0]).all():
                            curvature = 0
                            pass
                        elif (P[0,0] == P[:,0]).all() and (P[0,2] == P[:,2]).all():
                            curvature = 0
                            pass
                        else:
                            if (P[0,2] == P[:,2]).all():
                                r = circular_fitting_2d(P[:,0],P[:,1])
                                curvature = (1.0/(r*grid_mpc))
                                pass        

                            elif (P[0,1] == P[:,1]).all():
                                r = circular_fitting_2d(P[:,0],P[:,2])
                                curvature = (1.0/(r*grid_mpc))
                                pass

                            elif (P[0,0] == P[:,0]).all():
                                r = circular_fitting_2d(P[:,1],P[:,2])
                                curvature = (1.0/(r*grid_mpc))
                                pass        
                            else:
